...
void runGame(Game* pGame){
    ...
    while (isRunning){
        switch (pGame->state) {
            ...
            break;
            case ONGOING: handleOngoing(pGame, event, &isRunning, &right, &left, &currentPlatformY, &maxJumpHeight);
            break;
...
void handleMainMenu(Game* pGame, SDL_Event event, int* pMousePos){
    ...
    while (SDL_PollEvent(&event)){
        ...
        *pMousePos = getMousePos(pGame->quitButtonRect, *pMousePos, pGame->pQuitButton);
        ...
        // KRASHCAR P� MAC renderText(pGame->pStartButtonText);
        // KRASCHAR P� MAC renderText(pGame->pSettingsButtonText);
        // KRASCHAR P� MAC renderText(pGame->pQuitButtonText);
    }
}
...
void handleOngoing(Game* pGame, SDL_Event event, bool* pIsRunning, bool* pRight, bool* pLeft, float* pCurrentPlatformY, float* pMaxJumpHeight){
    ...
    movePlayer(pGame->pPlayer1, pGame->player1Rect, *pLeft, *pRight, pGame->windowWidth);
