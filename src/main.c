...
void handleOngoing(Game* pGame, SDL_Event event, bool* pIsRunning, bool* pLeft, bool *pRight, int *pTime, bool* pMute){
	...
	if (isHost) {
		GameplayData temp;
		// SERVER
		// H�r ska data h�mtas fr�n andra structar och l�ggas in i varabeln "temp".
		// Just nu har GameplayData definierats som att bara inneh�lla en array av Player-objekt.
		// Ni kan hitta den aktuella definitionen i "network.h".
		// Koden som flyttar data in i "temp" m�ste ligga precis h�r vid dessa kommentarer.
	...
	if (isHost) {
		// SERVER
		// Arrayen hos pGame->pClientCommands inneh�ller nu ett antal (0 �r m�jligt) kommandon,
		// d�r varje kommando f�rmedlar n�gon handling som en klient vill utf�ra. H�r ska dessa
		// handlingar appliceras p� v�r lokala (serverns) version av spelet. Definitionen av
		// hur ett kommando ser ut hittas i "network.h".
		// Rensa hela arrayen n�r alla kommandon har applicerats:
		*pGame->pClientCommands = (ClientCommand){0};
	} else {
		// KLIENT
		// Vid den h�r punkten har pGame->pGameplayData uppdaterats med data fr�n servern.
		// Denna data, som �r tillg�nglig via pGame->pGameplayData, ska nu l�ggas in i
		// de andra structarna (exempelvis pGame->pPlayers[i]).
		// Som sagt ser ni vad GameplayData kan inneh�lla genom att kolla i "network.h".
		// Koden m�ste ligga inom klammerparenteserna.
	}
